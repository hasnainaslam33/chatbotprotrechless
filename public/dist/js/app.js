...
function renderAiResult(answer, links=[], meta={}){
  const box = document.getElementById('result-box');
  if(!box) return;
  const urgency = extractUrgencyFromText(answer);
  const cls = riskClass(urgency);
  const costEstimate = meta.costEstimate || (getModuleKey() === 'cost-estimator' ? extractCostRangeFromText(answer) : '');
  ...
  box.innerHTML = `
    ...
    ${costCard}
    <div class="ai-structured-answer">${textToHtml(answer)}</div>
  `;
  if(links.length){
...
${context || 'No form details provided.'}`
    ...
    renderAiResult(result.answer, result.links || [], { costEstimate: costEstimate?.label });
